...
(function () {
  document.addEventListener('click', function (e) {
    ...
    const headerOffset = document.querySelector('.rd-navbar')
      ? document.querySelector('.rd-navbar').offsetHeight
      : 0;
...
document.addEventListener('DOMContentLoaded', function () {
    function loadB24FormOnce() {
        if (window.b24FormLoaded) return;
        window.b24FormLoaded = true;

        var anchor = document.getElementById('b24-form-anchor');
        if (!anchor) return;
		
		var dataB24form = anchor.getAttribute('data-b24-form');
		var dataB24loader = anchor.getAttribute('data-b24-form-loader');

        // 1. Скрипт с data-b24-form кладем ВНУТРЬ якоря
        var formScript = document.createElement('script');
        formScript.setAttribute('data-b24-form', dataB24form);
        formScript.setAttribute('data-skip-moving', 'true');
        anchor.appendChild(formScript);

        // 2. Загрузчик Bitrix24 можно повесить просто на body
        var loader = document.createElement('script');
        loader.async = true;
        loader.src = dataB24loader + '?' + (Date.now() / 180000 | 0);
